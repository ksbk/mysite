...
class SiteConfigSchema(BaseModel):
    ...
    navigation: list[dict[str, Any]] = Field(
    ...
    @classmethod
    def validate_navigation(cls, v: list[dict[str, Any]]) -> list[dict[str, Any]]:
        """Validate navigation menu structure."""
        validated = []
        for i, item in enumerate(v):
            if not isinstance(item, dict):
                raise ValueError(f"Navigation item {i} must be a dictionary")

            # Required fields
            if "label" not in item:
                raise ValueError(f"Navigation item {i} missing required 'label' field")
            if "url" not in item:
                raise ValueError(f"Navigation item {i} missing required 'url' field")

            # Validate types
            if not isinstance(item["label"], str):
                raise ValueError(f"Navigation item {i} 'label' must be string")
            if not isinstance(item["url"], str):
                raise ValueError(f"Navigation item {i} 'url' must be string")

            # Optional 'active' field defaults to False
            item.setdefault("active", False)
            if not isinstance(item["active"], bool):
                raise ValueError(f"Navigation item {i} 'active' must be boolean")
...
class SEOConfigSchema(BaseModel):
    ...
    def validate_canonical_url(cls, v: str | HttpUrl) -> str:
        ...
        if not url_str.startswith(("http://", "https://")):
            raise ValueError("Canonical URL must include protocol (http/https)")
        return url_str
    ...
    def validate_og_image(cls, v: str | HttpUrl) -> str:
        ...
        url_str = str(v)
        if not url_str.startswith(("http://", "https://")):
            raise ValueError("OG image URL must include protocol (http/https)")

        # Check for common image extensions
        valid_extensions = (".jpg", ".jpeg", ".png", ".gif", ".webp")
        if not any(url_str.lower().endswith(ext) for ext in valid_extensions):
            # Don't raise error, but warn that it might not work well
            pass
        return url_str
    ...
    def validate_google_verification(cls, v: str) -> str:
        ...
        # Google verification codes are typically 43-44 characters, alphanumeric + - and _
...
class ThemeConfigSchema(BaseModel):
    ...
    def validate_color(cls, v: str) -> str:
        """Validate color is a proper hex color."""
        if not v.startswith("#"):
